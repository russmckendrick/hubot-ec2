    ...
    ec2.describeInstances (getParams(msg.match[1])), (err, res) ->
      if err
        ...
        if servers.length == 0
          ...
          text =   "#{padRight('Environment', 20)} #{padRight('Name', 32)} #{padRight('State', 16)} #{padRight('Instance Id', 20)} #{padRight('Instance Type', 16)} #{padRight('Public IP', 20)} #{padRight('Availability Zone', 16)}\n"
          text +=  "------------------------------------------------------------------------------------------------------------------------------------------------\n"
          text += ("#{padRight(s.environment, 20)} #{padRight(s.name, 32)} #{padRight(s.state, 16)} #{padRight(s.instance_id, 20)} #{padRight(s.instance_type, 16)} #{padRight(s.public_ip, 20)} #{padRight(s.availability_zone, 16)}" for s in servers).join("\n")
          msg.send "```#{text}```"
